...
function programo_interpret($input, $convo_id) {
    $handle = prepare_curl_api_request(PROGRAM_O_API_URI, 'POST',
        array(
            'say' => $input,
            'bot_id' => 5,
            'format' => 'json',
    ...
    $response = perform_curl_request($handle);
    if($response === false) {
        error_log('Failed to perform Program-O request');
        return '';
    }
...
function telegram_program_combo($chat_id, $input) {
    $response = programo_interpret($input, "tlgrm-$chat_id");

    echo "Bot response '$response'" . PHP_EOL;

    if(strpos($response, 'BOTCMD') !== 0) {
...
function generate_results($chat_id, $lat, $lng) {
    $results = db_table_query("SELECT `word`, count(*) as `num` FROM `word_votes` WHERE CHAR_LENGTH(`word`) > 3 AND SQRT( POW(111.2 * (lat - $lat), 2) + POW(111.2 * ($lng - lng) * COS(lat / 57.3), 2)) < " . RESULT_DISTANCE . " group by `word` ORDER BY `num` DESC, RAND() LIMIT 7");

    if(!$results || sizeof($results) < 1) {
        telegram_send_message($chat_id, 'Non ci sono risultati intorno alla tua posizione attuale. Dimmi quando sei "pronto" per darmi i tuoi contributi.');
    }
    else {
        $response = "Le parole più popolari attorno a te:\n";
